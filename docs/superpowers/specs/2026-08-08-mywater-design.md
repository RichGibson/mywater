...
- **Precompute step**: a one-time (re-runnable) offline Python script using GeoPandas/Shapely that builds the `parcels` and `parcel_clusters` tables before first deploy. The running app never performs live geometric clustering — it only reads precomputed tables.
...
1. **Fetch** parcel geometry from the Lake County GeoHub (GeoJSON) and the CLOCWD service area boundary (published at clocwd.org/service-area-map), clipping parcels to just those inside the service area.
2. **Cluster** parcels into contiguous groups of roughly 8 along street frontage (walk each street segment, bucket consecutive parcels), producing each cluster's boundary geometry (union of member parcels) and a centroid snapped to the street midpoint of that group.
